...
const ListResponse = <T extends z.ZodTypeAny>(item: T) => z.object({
...
export const ListClientResponseSchema = ListResponse(ClientSchema);

export const ListContractResponseSchema = ListResponse(ContractSchema);

export const ListDepartmentsResponseSchema = ListResponse(DepartmentSchema);
...
export const DocumentSchema = z
    .string()
    .min(1, "Documento não pode ser vazio")
    .refine(
        (doc) =>
            /^\d{3}\.\d{3}\.\d{3}-\d{2}$/.test(doc) || // CPF
            /^\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}$/.test(doc), // CNPJ
        {
            message: "Documento deve estar no formato CPF ou CNPJ",
        },
    );
